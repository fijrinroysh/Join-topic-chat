...
package com.example.roysh.myapplication.autobahn;
...
import com.example.roysh.myapplication.autobahn.WebSocket.WebSocketConnectionObserver.WebSocketCloseNotification;
import com.example.roysh.myapplication.autobahn.WebSocketMessage.WebSocketCloseCode;
...
public class WebSocketConnection implements WebSocket {
	...
	public void disconnect() {
		Log.d(TAG, "Invoked disconnect.Current state: "+(mSocketThread==null?"null":mSocketThread.isRunning()));
	...
	protected boolean scheduleReconnect() {
		...
		boolean shouldReconnect = !mDisconnect && mPreviousConnection && (interval > 0);
		Log.d(TAG,"Trying for reconnection :"+shouldReconnect);
		if (shouldReconnect) {
	...
	protected void createWriter() {
		...
		Log.d(TAG, "WebSocket writer created and started."+mWebSocketWriter.getId());
	}
	...
	protected void createReader() {
		...
		Log.d(TAG, "WebSocket reader created and started."+mWebSocketReader.getId());
	}
	...
	public class SocketThread extends Thread {
		...
		public void startConnection() {	
			try {
				...
				if(factory==null){
					Log.w(TAG, "startConnection:: factory is null");
				}
			...
			if (mSocket == null) {
				...
				onClose(WebSocketCloseNotification.CANNOT_CONNECT,getFailureMessage());
			...
			Log.d(TAG, "startConnection complete."+Thread.currentThread().getId());
		}
		...
		public void stopConnection() {
			try {
				if(mSocket!=null){
					...
					Log.w(TAG, "Socket is already null");
				}
